...
impl<'a> Binding<'a> {
    ...
    fn get_vars(self) -> &'a Vars {
        match self {
            ...
            Binding::Potion(potion) => panic!("Unexpected vars check on potion: {}", potion.base.name),
...
impl ResolvedTarget {
    fn to_creature<'a>(self, battle_state: &'a BattleState, game_state: &'a GameState) -> &'a Creature {
        match self {
            ResolvedTarget::Player => {
                &game_state.player
            },
            ResolvedTarget::Monster(idx) => {
                &battle_state.monsters[idx as usize].creature
            },
...
pub fn eval_target(target: &Target, battle_state: &BattleState, binding: &Binding, action: &Option<GameAction>) -> ResolvedTarget {
    match target {
        Target::_Self => {
            match binding.get_monster() {
...
pub fn eval_amount(amount: &Amount, game_state: &GameState, battle_state: &BattleState, binding: &Binding) -> i16 {
    match amount {
        Amount::ByAsc(low, mid, high) => {
            match game_state.room {
                RoomType::HallwayFight => {
                    if game_state.asc >= 17 { *high }
                    else if game_state.asc >= 2 { *mid }
                    else { *low }
                },
                RoomType::Elite => {
                    if game_state.asc >= 18 { *high }
                    else if game_state.asc >= 3 { *mid }
                    else { *low }
                },
                RoomType::Boss => {
                    if game_state.asc >= 19 { *high }
                    else if game_state.asc >= 4 { *mid }
                    else { *low }
                }
        ...
        Amount::N => {
            binding.get_vars().n as i16
        },
        Amount::NegX => {
            binding.get_vars().x as i16 * -1
        },
        Amount::OrbCount => {
        ...
        Amount::Upgradable(low, high) => {
            match binding {
                Binding::Buff(_, buff) => panic!("Unexpected upgradeable check on buff: {}", buff.base.name),
                Binding::Card(card) => {
                    if card.upgrades == 0 { *low }
                    else { *high }
                },
                Binding::Monster(monster) => panic!("Unexpected upgradeable check on monster: {}", monster.base.name),
                Binding::Potion(potion) => panic!("Unexpected upgradeable check on potion: {}", potion.base.name),
                Binding::Relic(_) => {
                    if game_state.relics.contains_key(crate::models::relics::SACRED_BARK) {*high}
                    else { *low }
                }
            }
        },
        Amount::X => {
            binding.get_vars().x as i16
        },
        Amount::Any => {
...
pub fn eval_condition<'a>(condition: &Condition, battle_state: &BattleState, game_state: &GameState, binding: &Binding, action: &Option<GameAction>) -> bool {
    match condition {
        Condition::Act(act) => {
            &game_state.act == act
        },
        ...
        Condition::Attacking(target) => {
            match eval_target(target, battle_state, binding, action) {
                ResolvedTarget::Monster(idx) => {
                    match battle_state.monsters[idx as usize].intent {
                        Intent::Attack => true,
        ...
        Condition::Buff(target, buff) => {
            let creature = eval_target(target, battle_state, binding, action).to_creature(battle_state, game_state);
            creature.buffs.contains_key(buff)
        },
        Condition::BuffX(target, buff, x) => {
            let val = eval_amount(x, game_state, battle_state, binding) as u8;
            let creature = eval_target(target, battle_state, binding, action).to_creature(battle_state, game_state);
            creature.buffs.get(buff).map(|a| a.vars.x >= val).unwrap_or(false)
        },
        Condition::Custom => {
            match binding.get_name() {
        ...
        Condition::Dead(target) => {
            eval_target(target, battle_state, binding, action).to_creature(battle_state, game_state).hp == 0
        },
